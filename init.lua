...
local __pkg  = "Boundary Windows Performance Counter Plugin"
local __ver  = "Version 1.0"
local __tags = "lua,plugin,wpc,windows"
...
-- Configuration.
local _parameters = (boundary and boundary.param) or {}

_parameters.pollInterval = 
  (_parameters.pollInterval and tonumber(_parameters.pollInterval)>999  and tonumber(_parameters.pollInterval)) or
  pollInterval;
...
function typePerfParser( err, stdout, stderr )
  ...
  -- create Boundary events for errors
  print("_bevent:"..__pkg..":"..__ver..":"..(err or stderr).."|t:error|tags:"..__tags)
  return
  end
...
for idx,counter in ipairs(counters) do 
  ...
  if (pdh.PdhValidatePathA(ffi.cast("char *",path)) == 0) then
    ...
    print("_bevent:"..__pkg..":"..__ver..":Can't find a valid localized path for "..counter.metric.."|t:error|tags:"..__tags)
    counters[idx] = 0
  end
end
